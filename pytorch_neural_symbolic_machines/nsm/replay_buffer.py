...
class DDSReplayBuffer(object):
    ...
    def replay(self, environments, sample_categories, n_samples=1, use_top_k=False, truncate_at_n=0, replace=True,
               consistency_model=None, constraint_sketches=None, debug_file=None):
        
        
        select_env_names = set([e.name for e in environments]) # why different from list?
        sys.stdout.flush()
        # category_set = 
        # assert(len(select_env_names) == len(environments))
        trajs = []
        traj_cats = []

        # Collect all the trajs for the selected environments.
        # we had their trajectories in the buffer, but categories we can use from supplied. 
        new_sample_cats = []
        for ename in select_env_names:
            for cat, env in zip(sample_categories, environments):
                if env.name == ename:
                    new_sample_cats.append(cat)
                    break
        
        sample_categories = new_sample_cats
        assert len(sample_categories) == len(select_env_names)
        ...
        assert len(trajs) == len(traj_cats), 'should be same since populated together %d %d' % (len(trajs), len(traj_cats))
        ...
        sample_categories = traj_cats

        assert len(samples) == len(sample_categories), 'All should be same: %d %d %d' % (len(samples), len(sample_categories), len(trajs))

        env_sample_dict = dict()
        
        for sample, category in zip(samples, sample_categories):
            env_name = sample.trajectory.environment_name
            env_sample_dict.setdefault(env_name, []).append((sample, category))

        replay_samples = []
        replay_sample_categories = []
        for env_name, sample_pairs in env_sample_dict.items():
            samples = [paired[0] for paired in sample_pairs]
            categories = [paired[1] for paired in sample_pairs]
            ...
            if use_top_k: # not handled. 
                ...
                selected_categories = [categories[i] for i in selected_sample_indices]
            ...
            replay_sample_categories += selected_categories

        assert len(replay_samples) == len(replay_sample_categories), print('assertion failed')
        sys.stdout.flush()
        return replay_samples, replay_sample_categories
